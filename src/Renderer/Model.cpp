...
namespace Diffuse {
	...
	void Texture::fromglTfImage(tinygltf::Image& gltfimage, TextureSampler textureSampler, GraphicsDevice* device, VkQueue copyQueue)
	{
		this->device = device;
		...
		if (vkCreateImage(device->Device(), &imageCreateInfo, nullptr, &image)) {
			throw std::runtime_error("failed to load texture image!");
		}
		...
		if (vkAllocateMemory(device->Device(), &memAllocInfo, nullptr, &deviceMemory)) {
			throw std::runtime_error("failed to load texture image!");
		}
		if (vkBindImageMemory(device->Device(), image, deviceMemory, 0)) {
			throw std::runtime_error("failed to load texture image!");
		}
		...
		FlushCommandBuffer(device->Device(), copyCmd, copyQueue, device->CommandPool(), true);

		vkFreeMemory(device->Device(), stagingMemory, nullptr);
		vkDestroyBuffer(device->Device(), stagingBuffer, nullptr);

		// Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
		VkCommandBuffer blitCmd = CreateCommandBuffer(device->Device(), VK_COMMAND_BUFFER_LEVEL_PRIMARY, device->CommandPool(), true);
		...
		FlushCommandBuffer(device->Device(), blitCmd, copyQueue, device->CommandPool(), true);
		...
		if (vkCreateImageView(device->Device(), &viewInfo, nullptr, &view)) {
			throw std::runtime_error("failed to load texture image!");
		}
	...
	Mesh::Mesh(GraphicsDevice* device, glm::mat4 matrix) {
		...
		if (vkMapMemory(device->Device(), uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped)) {
			throw std::runtime_error("failed to load texture image!");
		}
	...
	void Model::loadTextures(tinygltf::Model& gltfModel, GraphicsDevice* device, VkQueue transferQueue)
	{
		for (tinygltf::Texture& tex : gltfModel.textures) {
			...
			Texture2D* texture;
			//texture = new Texture2D(image, textureSampler, device, transferQueue);
			texture = new Texture2D(image, device);
	...
	void Model::loadFromFile(std::string filename, GraphicsDevice* device, VkQueue transferQueue, float scale)
	{
	...
	void Model::calculateBoundingBox(Node* node, Node* parent) {
		...
		if (parent != nullptr) {
			...
			parentBvh = BoundingBox(dimensions.min, dimensions.max);
		}
