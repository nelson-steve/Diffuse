...
namespace Diffuse {
	...
	void Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
	{
		...
		if(vkAllocateDescriptorSets(device->Device(), &descriptorSetAllocInfo, &descriptorSet));
		std::vector<VkDescriptorImageInfo> imageDescriptors{};
		std::vector<VkWriteDescriptorSet> writeDescriptorSets{};
		if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
			imageDescriptors.push_back(baseColorTexture->m_descriptor);
			...
			writeDescriptorSet.pImageInfo = &baseColorTexture->m_descriptor;
			writeDescriptorSets.push_back(writeDescriptorSet);
		}
		if (normalTexture && descriptorBindingFlags & DescriptorBindingFlags::ImageNormalMap) {
			imageDescriptors.push_back(normalTexture->m_descriptor);
			VkWriteDescriptorSet writeDescriptorSet{};
			writeDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			writeDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
			writeDescriptorSet.descriptorCount = 1;
			writeDescriptorSet.dstSet = descriptorSet;
			writeDescriptorSet.dstBinding = static_cast<uint32_t>(writeDescriptorSets.size());
			writeDescriptorSet.pImageInfo = &normalTexture->m_descriptor;
			writeDescriptorSets.push_back(writeDescriptorSet);
		}
	...
	/*
		glTF mesh
	*/
	Mesh::Mesh(GraphicsDevice* device, glm::mat4 matrix) {
		this->device = device;
		this->uniformBlock.matrix = matrix;
		vkUtilities::CreateBuffer(
			sizeof(uniformBlock),
			...
			device->Device());
		if (vkMapMemory(device->Device(), uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped) != VK_SUCCESS) {
...
*/
	...
	Texture2D* Model::getTexture(uint32_t index)
	{
	...
	void Model::createEmptyTexture(VkQueue transferQueue)
	{
		...
		emptyTexture->m_graphics_device = device;
		emptyTexture->m_width = 1;
		emptyTexture->m_height = 1;
		emptyTexture->m_layers= 1;
		emptyTexture->m_mipLevels = 1;

		size_t bufferSize = emptyTexture->m_width * emptyTexture->m_height * 4;
		...
		if(vkCreateBuffer(device->Device(), &bufferCreateInfo, nullptr, &stagingBuffer));
		...
		memAllocInfo.memoryTypeIndex = vkUtilities::FindMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, device->PhysicalDevice());
		if(vkAllocateMemory(device->Device(), &memAllocInfo, nullptr, &stagingMemory));
		if(vkBindBufferMemory(device->Device(), stagingBuffer, stagingMemory, 0));

		// Copy texture data into staging buffer
		uint8_t* data;
		if(vkMapMemory(device->Device(), stagingMemory, 0, memReqs.size, 0, (void**)&data));
		...
		bufferCopyRegion.imageExtent.width = emptyTexture->m_width;
		bufferCopyRegion.imageExtent.height = emptyTexture->m_height;
		...
		imageCreateInfo.extent = { emptyTexture->m_width, emptyTexture->m_height, 1 };
		imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
		if(vkCreateImage(device->Device(), &imageCreateInfo, nullptr, &emptyTexture->m_texture_image));

		vkGetImageMemoryRequirements(device->Device(), emptyTexture->m_texture_image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = vkUtilities::FindMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, device->PhysicalDevice());
		if(vkAllocateMemory(device->Device(), &memAllocInfo, nullptr, &emptyTexture->m_texture_image_memory));
		if(vkBindImageMemory(device->Device(), emptyTexture->m_texture_image, emptyTexture->m_texture_image_memory, 0));
		...
		VkCommandBuffer copyCmd = CreateCommandBuffer(device->Device(), VK_COMMAND_BUFFER_LEVEL_PRIMARY, device->CommandPool(), true);
		SetImageLayout(copyCmd, emptyTexture->m_texture_image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
		vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture->m_texture_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
		SetImageLayout(copyCmd, emptyTexture->m_texture_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
		FlushCommandBuffer(device->Device(), copyCmd, transferQueue, device->CommandPool());
		emptyTexture->m_imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		...
		if(vkCreateSampler(device->Device(), &samplerCreateInfo, nullptr, &emptyTexture->m_texture_sampler));
		...
		viewCreateInfo.image = emptyTexture->m_texture_image;
		if(vkCreateImageView(device->Device(), &viewCreateInfo, nullptr, &emptyTexture->m_texture_image_view));

		emptyTexture->m_descriptor.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		emptyTexture->m_descriptor.imageView = emptyTexture->m_texture_image_view;
		emptyTexture->m_descriptor.sampler = emptyTexture->m_texture_sampler;
	}
	...
	*/
	Model::~Model()
	{
		...
		if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
			vkDestroyDescriptorSetLayout(device->Device(), descriptorSetLayoutUbo, nullptr);
			descriptorSetLayoutUbo = VK_NULL_HANDLE;
		}
		if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
			vkDestroyDescriptorSetLayout(device->Device(), descriptorSetLayoutImage, nullptr);
	...
	void Model::loadImages(tinygltf::Model& gltfModel, GraphicsDevice* device, VkQueue transferQueue)
	{
		for (tinygltf::Image& image : gltfModel.images) {
			Texture2D* texture;
			// TODO: FIX
			//texture = new Texture2D(image, path, device, transferQueue);
	...
	void Model::loadFromFile(std::string filename, GraphicsDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
	{
		...
		FlushCommandBuffer(device->Device(), copyCmd, transferQueue, device->CommandPool(), true);
		...
		if(vkCreateDescriptorPool(device->Device(), &descriptorPoolCI, nullptr, &descriptorPool) != VK_SUCCESS) {
			throw std::runtime_error("Failed to create descriptor pool");
		}
		...
		// Descriptors for per-node uniform buffers
		{
			...
			if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
				...
				if(vkCreateDescriptorSetLayout(device->Device(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
			}
			for (auto node : nodes) {
		...
		// Descriptors for per-material images
		{
			...
			if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
				std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings{};
				if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
					setLayoutBindings.push_back(InitDescriptorSetLayoutBinding(static_cast<uint32_t>(setLayoutBindings.size()), 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT));
				}
				if (descriptorBindingFlags & DescriptorBindingFlags::ImageNormalMap) {
					setLayoutBindings.push_back(InitDescriptorSetLayoutBinding(static_cast<uint32_t>(setLayoutBindings.size()), 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT));
				}
				...
				descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
				descriptorLayoutCI.pBindings = setLayoutBindings.data();
				if(vkCreateDescriptorSetLayout(device->Device(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
			}
			for (auto& material : materials) {
				if (material.baseColorTexture != nullptr) {
	...
	*/
	...
	void Model::prepareNodeDescriptor(Node* node, VkDescriptorSetLayout descriptorSetLayout) {
		if (node->mesh) {
			...
			descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
			descriptorSetAllocInfo.descriptorSetCount = 1;
			if(vkAllocateDescriptorSets(device->Device(), &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
